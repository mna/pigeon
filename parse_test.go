...
var invalidParseCases = map[string]string{
	"": `file:1:0 (0): rule Whitespace: expected [ \t\r], got ""`,
	// TODO : this actually failed after the identifier, but because the ffp
	// is recorded only on match failures, it is on the IdentifierPart, that
	// calls IdentifierStart (and IdentifierPart is actually optional, with a *)
	"a":          `file:1:1 (1): rule IdentifierStart: expected [\pL_], got ""`,
	"abc":        `file:1:3 (3): rule IdentifierStart: expected [\pL_], got ""`,
	" ":          `file:1:1 (1): rule Whitespace: expected [ \t\r], got ""`,
	`a = +`:      `file:1:5 (4): rule Whitespace: expected [ \t\r], got "+"`,
	`a = *`:      `file:1:5 (4): rule Whitespace: expected [ \t\r], got "*"`,
	`a = ?`:      `file:1:5 (4): rule Whitespace: expected [ \t\r], got "?"`,
	"a ←":        `file:1:3 (5): rule Whitespace: expected [ \t\r], got ""`,
	"a ← b\nb ←": `file:2:3 (13): rule Whitespace: expected [ \t\r], got ""`,
	"a ← nil:b":  "file:1:5 (6): rule Identifier: identifier is a reserved word",
	"\xfe":       "file:1:0 (0): invalid encoding",
	"{}{}":       `file:1:3 (2): rule Whitespace: expected [ \t\r], got "{"`,
